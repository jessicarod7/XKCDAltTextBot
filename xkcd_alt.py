...
class Twitter():
    ...
    def get(self):
        ...
        for attempt in range(6):
            ...
            a = -1 # Indicates which Tweet from search to be used
            for i in range(len(alt['statuses'])): # Find most recent XKCDAltTextBot API Tweet
                if re.search(r'XKCDAltTextBot', str(alt['statuses'][i]['source'])) is None:
                    continue # This tweet was not generated by the bot itself
                else: # This tweet was made with the Twitter API
                    try:
                        if alt['statuses'][i]['in_reply_to_status_id'] == alt['statuses'][i+1]['id']:
                            continue # This tweet is part of a bot Tweetstorm
                        else:
                            a = i # This is the first Tweet in the most recent API Tweetstorm
                    except IndexError: # Reached end of search
                        continue
                    break

            if alt['statuses'][a]['in_reply_to_status_id'] is None or \
            comic['statuses'][0]['id'] is None or a is -1:
                print('Twitter search failed: No Tweet found')
                del alt_payload, comic_payload, alt_raw, comic_raw, alt, comic, a
                return 'crash' # Enter log protection mode
            
            if alt['statuses'][a]['in_reply_to_status_id'] == comic['statuses'][0]['id']:
                # This tweet has already been replied to
                del alt_payload, comic_payload, alt_raw, comic_raw, alt, comic, a
                return None # Sleep for 60 seconds
            else:
                # This tweet has not been replied to
                del alt_payload, comic_payload, alt_raw, comic_raw, alt, a
